...
class EXPRESSOutlineTreeProvider extends DefaultOutlineTreeProvider {
	...
	def Object _text(Entity e) {
		...
		s.append(if(str.length > 0) " : " + str.substring(0, str.length - 1) else "", 
	...
	def Object _text(Type e) {
		val StyledString s = new StyledString(if(e.datatype instanceof EnumType) "Enum" else "Type", 
			stylerFactory.createXtextStyleAdapterStyler(typeTextStyleIndication(9)));
		s.append(" "+e.name);
	...
	def Object _text(Attribute a) {		
		...
		val StyledString s = new StyledString(
			if(a.expression!=null) "derived " else "" 
			+ if(a.opposite!=null) "inverse " else ""
