...
abstract class AbstractXcoreGeneratorTest {
	...
	/**
	 * Parses a char sequence into the given schema object.
	 */
	def parseSchema(CharSequence schemaCharSequence) {

		parseHelper.parse(schemaCharSequence, resourceSet);
	}
	...
	def validateXCore(CharSequence xcoreModel) {
		...
		if (null != packageInstance) {
			...
			val issues = xcoreResourceValidator.validate(xtextResource, CheckMode.EXPENSIVE_ONLY,
